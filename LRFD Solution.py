...
def solve():
    ...
    design_strength[np.logical_and(logic_1, compact_logic)] = 0.9*f_y*Zx[np.logical_and(logic_1, compact_logic)]
    ...
    design_strength[np.logical_and(logic_2, compact_logic)] = 0.9*design[np.logical_and(logic_2, compact_logic)]
    ...
    design_strength[np.logical_and(logic_3, compact_logic)] = 0.9*(Fcr*Sx)[np.logical_and(logic_3, compact_logic)]
...
def grab_val(event):
    ...
    design_strength_text.config(text=f"Design Strength = {np.round(values[8]/1000, 4)}")
...
mainWindow = Tk()
mainWindow.title("W-Shaped Steel Calculator (LRFD - Metric Units)")
